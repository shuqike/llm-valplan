...
from mcts import reasoning_mcts_search


def validate_plan(domain, instance, plan_file):
    '''Model-parallel communication stuff for local llm model
    '''
    val_path = os.getenv("VAL") # Refer to https://github.com/karthikv792/LLMs-Planning/tree/main/planner_tools/VAL
...
class ReasoningTasks():
    ...
    def __init__(self, verbose=False, model_name="LLaMA", data_path=""):
        # self.engine = engine
        ...
        self.log_file = "logs/interactive.log"

        assert model_name in openai.Model.list()
    ...
    def run_mcts(self, config_file, name="", prompts="", rollouts=10, max_depth=4, alpha=0.5, prompt_path=""):
        ...
        for i in range(n_files):
            ...
            query += fill_template(*parsed_instance_to_text_blocksworld(problem, False, self.data)) + "\n" # Refer to https://github.com/karthikv792/LLMs-Planning/blob/9bd247838ec314f048f6a37a147201e74d64572d/plan-bench/utils/pddl_to_text.py#L59
            
            trajs, tree, trees = reasoning_mcts_search( # TODO: remove unwanted parameters in this function call
                f'I have that, {INIT.strip()}.', 
            ...
            actions = re.findall('\[ACTION \d\](.*)', trajs[-1]) # TODO: deal with trajs
            _, lm_plan = text_to_plan_blocksworld('\n'.join(actions), problem.actions, self.lm_plan_file, self.data)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', type=str, default='text-davinci-003', help='Model to use')
    # verbose: if we want to print something in run-time
